...
class LogicalPictures:
    ...
    def get_indeksy_with_smallest_domain(self):
        smallest_domain = 100000000
        indeksy = (0, 0)
        for i, j in self.queue:
            if self.rows[i].domain and self.columns[j].domain:
                if len(self.rows[i].domain) < smallest_domain:
                    smallest_domain = len(self.rows[i].domain)
                    indeksy = (i, j)

                if len(self.columns[j].domain) < smallest_domain:
                    smallest_domain = len(self.columns[j].domain)
        return indeksy
    ...
    def solve(self):
        ...
        while stack:
            current = stack[-1]
            cur_state = current[0]
            cur_state.wnioskowanie()

            if cur_state.every_domain_is_not_empty():
                if cur_state.is_solved():
                    cur_state.fill_board()
                    cur_state.print_result_to_file()
                    return True
                else:
                    new_state = copy.deepcopy(cur_state)
                    indeksy = new_state.get_indeksy_with_smallest_domain()
                    new_state.queue.remove(indeksy)
                    new_state.set_field(indeksy[0], indeksy[1], 0)
                    stack.append((new_state, 0))
            else:
                while stack[-1][1] != 0:
                    stack.pop()
                stack.pop()
                new_state = copy.deepcopy(stack[-1][0])
                indeksy = new_state.get_indeksy_with_smallest_domain()
                new_state.queue.remove(indeksy)
                new_state.set_field(indeksy[0], indeksy[1], 1)
                stack.append((new_state, 1))
