...
(define-type Exp
  ...
  (varE [x : Symbol])
  (letE [binds : (Listof (Symbol * Exp))] [e : Exp])
  (letE* [binds : (Listof (Symbol * Exp))] [e : Exp]))   
  ...
  (cond
   ...
   [(s-exp-match? `{let* {{SYMBOL ANY} ... } ANY} s)
     (letE* (map (lambda (x) (pair (s-exp->symbol (first (s-exp->list x)))  (parse (second (s-exp->list x)))))
            ...
            (parse (third (s-exp->list s))))]
   [(s-exp-match? `{let SYMBOL ANY ANY} s)
     (letE (list (pair (s-exp->symbol (second (s-exp->list s)))
           (parse (third (s-exp->list s)))))
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(varE x)
     (lookup-env x env)]
    [(letE bin e1)
      (eval e1 (foldl (lambda (x en) (extend-env en (fst x) (eval (snd x) env))) env bin))]
    [(letE* bin e1)
     (if (empty? bin)
         (eval e1 env)
         (let ([v1 (eval (snd (first bin)) env)])
           (eval (letE* (rest bin) e1)
                 (extend-env env (fst (first bin)) v1))))]))
