...
(define (min lista w)
  (if (null? lista)
     w (max (cdr lista) (mniejsza w (first lista)))))
...
(my-append (list 1 2 3 4 5) (list))
(define g (my-append (list 1 2 3 4 5) (list)))
(my-append (list)  g)
...
(define (select lista)
  (define maly (min lista -inf.0))
  (define (iter wynik lista najmniejszy)
  (cond [(= (car lista) najmniejszy) (cons (car lista) (cons wynik (cdr lista)))]
        [else (iter (cons wynik (car lista)) (cdr lista) najmniejszy)]))
  (iter list (cdr lista) maly))

(select (list 3 4 5 2 3 4 1 9 4))