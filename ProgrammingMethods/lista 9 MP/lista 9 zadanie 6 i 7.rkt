...
;parse-op0 i parse-op1 zwracaja sparsowany operator i reszte wyrazenia
(define (parse-op0 ss)
...
(define (parse-exp1 ss) ; najwyzszy priorytet
  (type-case (Listof S-Exp) ss
    [empty (parse-err)]
    [(cons s rest)
     (cond
  ...
  (type-case (ParseResult Exp) (parse-exp1 ss)
    ...
    [(parse-ok e1 rest)
     (type-case (ParseResult Op) (parse-op1 rest)
       ...
       [(parse-ok op rest2)
        (type-case (ParseResult Exp) (parse-exp2 rest2)
          [(parse-err) (parse-err)]
          [(parse-ok e2 rest3) (parse-ok (exp-op op e1 e2) rest3)])])]))


(define (parse-exp0 ss) ;najnizsze priorytety
