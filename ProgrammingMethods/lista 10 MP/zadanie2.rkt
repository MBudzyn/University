  ...
  (type-case Exp e
    ...
    [(opE op e1 e2)
     (append (compile e1)
             (append (compile e2)
                     (list (opI op))))]))
;--------------------------------------------pomocnicze ostatni i bez ostatniego zadanie2
...
(define (remove-last [lst : (Listof Exp)]) : (Listof Exp)
  (cond
    [(empty? lst) (error 'remove-last "empty list")]
    [(empty? (rest lst)) empty]
    [else (cons (first lst) (remove-last (rest lst)))]))

(define (get-last [lst : (Listof Exp)]) : Exp
  (cond
    [(empty? lst) (error 'get-last "empty list")]
    [(empty? (rest lst)) (first lst)]
    [else (get-last (rest lst))]))

(define (polaczexp [l : (Listof Exp)] [op1 : Op]) : (Listof Exp)
  (let*
      ([zmienna1 (get-last l)]
       [lista1 (remove-last l)]
       [zmienna2 (get-last lista1)]
       [lista2 (remove-last lista1)])
    (append lista2  (list (opE op1 zmienna2 zmienna1)))))
         

(define (decompile [c : Code] [l : (Listof 'a)]) : Exp
  (type-case (Listof Instr) c
    [(cons a rest)
      (type-case Instr a
        [(pushI b) (decompile rest (append l (list (numE b))))]
        [(opI b) (decompile rest (polaczexp l b))])]
    [empty (first l)]))

(define (decompilewl [c : Code]) : Exp
  (decompile c '()))
    
(compile (decompilewl (list (pushI 2) (pushI 3) (opI (mul)) (pushI 5) (pushI 8) (opI (add)) (opI (add)))))
