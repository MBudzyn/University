...
(define (unionS [s1 : (Setof 'a)] [s2 : (Setof 'a)]) : (Setof 'a) ; suma
  (append (filter (λ (x) (not (member x s2))) s1) s2)) 

(define (removeS [x : 'a] [s : (Setof 'a)]): (Setof 'a) ; zbior s bez x
  (filter (λ (y) (not (eq? x y))) s))

(define (subsetS [s1 : (Setof 'a)] [s2 : (Setof 'a)]) : Boolean ; zawieranie s1 w s2
  (empty? (filter (λ (x) (not (member x s2))) s1)))

(define (equalS [s1 : (Setof 'a)] [s2 : (Setof 'a)]) : Boolean ; podwojne zawieranie
  (and (subsetS s1 s2) (subsetS s2 s1)))
  
(define (fromList [xs : (Listof 'a)]) : (Setof 'a) ; zamiana listy na zbiór co w naszym wypadku jest zbędne bo interpretujemy liste jako zbiór
  (foldr (λ (x s) (unionS (singletonS x) s))
         (emptyS)
         xs))

(define (fv [e : Exp]) : (Setof Symbol); zwraca zbiór zmiennych wolnych z wyrazenia exp
  (type-case Exp e
    [(numE n) (emptyS)]
    [(trueE) (emptyS)]
    [(falseE) (emptyS)]
...
;; precondition: a is a closed expression ; zamiana zmiennych x w wyrazeniu e na wyrazenie a
